...
export const getHasUppercaseLetters = () => {
  var lowerCaseString = stringWithUppercaseLetters.toLowerCase();
  if (lowerCaseString === stringWithUppercaseLetters) return false;
  else return true;
};
...
export const convertPascalCaseToSnakeCase = () => {
  var snake_case_variable_name = pascalCaseVariableName.replace(
    /([A-Z])/g,
    "_$1"
  );
  return snake_case_variable_name.toLowerCase().substring(1);
};
